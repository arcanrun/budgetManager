...
class HistoryTest(unittest.TestCase):
    ...
    def test_add_to_history(self):
        ...
        self.history.addToHistory('+', 50, self.app.budget, self.app.timeLine.now)
        res = self.history.getHistoryDict()
        self.assertEqual(res, {'2018-09-10': ['+budget:50.0']})

        self.history.addToHistory('+', 30,self.app.budget, self.app.timeLine.now)
        res = self.history.getHistoryDict()
        self.assertEqual(res, {'2018-09-10': ['+budget:50.0', '+budget:30.0']})

        self.history.addToHistory('-', 100,self.app.commonBudget, self.app.timeLine.now)
        res = self.history.getHistoryDict()
        self.assertEqual(res, {'2018-09-10': ['+budget:50.0', '+budget:30.0', '-common:100.0']})

        self.history.addToHistory('-', 2, self.app.funBudget, self.app.timeLine.now)
        res = self.history.getHistoryDict()
        self.assertEqual(res, {'2018-09-10': ['+budget:50.0', '+budget:30.0', '-common:100.0', '-fun:2.0']})

        self.history.addToHistory('+', 10, self.app.investBudget, self.app.timeLine.now)
        res = self.history.getHistoryDict()
        ...
        self.history.addToHistory('+', 9, self.app.investBudget, self.app.timeLine.now)
        res = self.history.getHistoryDict()
        ...
        self.history.addToHistory('-',18, self.app.commonBudget, self.app.timeLine.now)
        res = self.history.getHistoryDict()
        self.assertEqual(res, {
            '2018-09-10': [
                '+budget:50.0',
